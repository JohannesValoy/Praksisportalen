...
async function fetchInternshipAgreementByID(
  id: number,
...
async function fetchInternshipAgreementByIDList(
  idList: number[],
...
async function fetchInternshipAgreementByInternshipID(
  internshipsID: number[],
): Promise<Map<number, InternshipAgreementObject[]>> {
  const query = await DBclient.select("id")
    .from<InternshipAgreementTable>("internshipAgreements")
    .whereIn("internship_id", internshipsID);
  const agreementIds = query.map((agreement) => agreement.id);
  const agreements = (
    await fetchInternshipAgreementByIDList(agreementIds)
  ).values();
  const internshipAgreements: Map<number, InternshipAgreementObject[]> =
    new Map();
...
export {
  fetchInternshipAgreementByID,
  fetchInternshipAgreementByInternshipID,
  fetchInternshipAgreementByIDList,
};
