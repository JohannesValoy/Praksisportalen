...
class PageRequest {
    ...
    static fromRequest(request: NextRequest): PageRequest {
        const page = request.nextUrl.searchParams.get("page") ? parseInt(request.nextUrl.searchParams.get("page")) : 0;
        const size = request.nextUrl.searchParams.get("size") ? parseInt(request.nextUrl.searchParams.get("size")) : 10;
...
export class PageResponse extends PageRequest {
    ...
    constructor(PageRequest: PageRequest, elements: any[]) {
        ...
        this.elements = elements.slice(this.page * this.size, this.page * this.size + this.size);
...
export class UserPageRequest extends PageRequest {
    ...
    constructor(page: number, size: number, sort: string, roles: string[]) {
        if (['id', 'username', 'email', 'role'].indexOf(sort) === -1){
            sort = 'id';
        }
        super(page, size, sort);
        if (roles.length === 0 && roles.every(role => ['admin', 'employee', 'coordinator', 'student'].indexOf(role) === -1)){
    ...
    static fromRequest(request : NextRequest) {
        ...
        let roles = request.nextUrl.searchParams.get('role') ? request.nextUrl.searchParams.get('role').split(',') : [];
        return new UserPageRequest(page.page, page.size, page.sort, roles);
    }

    static default(): UserPageRequest {
        return new UserPageRequest(0, 10, 'id', ['admin', 'employee', 'coordinator', 'student']);
    }
}
